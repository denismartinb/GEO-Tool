...
3. **Only one project was ever inspected.** The journeys took whichever project
   ...
   skipping loudly if none does (log §138). `PILOT_SECOND_PROJECT` no longer
   exists.
...
| Onboarding (tour) | `onboarding.md` | `landing-hero-tour` + reapertura desde «¿Qué es el GEO?» (`onboarding-tour.spec.ts`); «sale solo en el primer acceso y no vuelve tras recargar» sólo bajo `--journeys write` (`journeys/write/onboarding-tour-first-run.spec.ts`) | Completa para landing y reapertura en el set por defecto. El «primer acceso» de `/dashboard` quedó fuera del set por defecto desde ONBOARDING-TOUR-PERSIST-1 (2026-08-25): la marca de «ya visto» vive en `profiles`, compartida por toda cuenta piloto, así que forzarla a «no visto» es una escritura de producto y el piloto siempre-on es estrictamente de lectura |
