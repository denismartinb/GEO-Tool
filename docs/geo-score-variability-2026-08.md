...
Mismo aviso, para que nadie lea el ADR 0030 como "la variabilidad está
arreglada":
...
- La Fase 2 dependía de que se levantara el tope de extracción (el antiguo
  `MAX_EXTRACTION_RESULTS` = 20). Mientras ese tope estuvo por debajo del
  número de filas de un escaneo, las filas sobrantes conservaban su
  `brand_mentioned` ingenuo — sin alias (Fase −1) y sin verificar (ADR 0021) —
  así que **más muestra era más denominador sin verificar**. Era la única
  dependencia dura de esta fase, y quedó **resuelta por
  EXTRACTION-RELIABILITY-1 (ADR 0029), mergeada el 2026-08-04**. Los escaneos
  anteriores a esa fecha se calcularon sobre una muestra truncada en silencio y
  no son comparables con los posteriores.
