...
| **C** | Anti-relleno, no-autopromoción, contenido visible del schema (F4, F5) | **implementada** (log §127) — C1 con guarda en servidor; C2 y C3 sólo en el prompt y declaradas blandas. La grafía de marca (F6) NO entra: sale del campo `brand` del proyecto, se arregla editándolo, no en código |
