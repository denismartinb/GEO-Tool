...
| 9 | ASYNC-SCAN-1 ⚠️ | 🟡 En curso (1c hecho, 1a en PR, 1b diseño hecho + implementación en PR) | #231, #232, #263 | 2026-07-25 | Task Intake de la Fase 9 aprobado 2026-07-17, dividida en tres: **1a CRON-SCALE** (sweep diario auto-encadenado, ADR-0016, sin schema) en PR #231 pendiente de Human Gate; **1c ASYNC-LAUNCH** (lanzamiento manual no bloqueante + mensaje honesto de escaneo activo, addendum ADR-0003) ✅ hecho, PR #232 mergeada 2026-07-18 tras smoke real del fundador; **1b NOTIF-SERVER** (notificaciones server-side, schema+RLS): diseño técnico completo ✅ mergeado 2026-07-25 en `docs/specs/notifications/notifications-v1.md` (PR #263) — tabla `notifications` propia, escritura en el evento en vez de derivación en lectura, `owner_user_id` denormalizado, catálogo de 8 tipos en 4 fases; el intercambio deja el layout del dashboard con una consulta menos que hoy. **Fase 1a implementada** (Task Intake aprobado 2026-07-25): migración `0021_notifications.sql`, módulo `lib/notifications/*` (`emitNotification` idempotente vía upsert + índice único sobre `owner_user_id, dedupe_key`, nunca lanza), y los 4 primeros tipos cableados en `lib/scan/executor.ts` (`scan_completed`, `scan_failed`, `gap_resolved` agregado por escaneo, `gap_pending` en el cruce exacto de 3 escaneos consecutivos abiertos, como mucho uno por escaneo priorizando el de mayor impacto). Corrección sobre el propio Task Intake: se documentaron 3 puntos de escritura de `scan_runs.status='failed'` en el ejecutor, la revisión del código confirmó solo 2 reales (el bloque "no hay jobs" y el `catch` general) — ambos cubiertos. Owner de cada notificación resuelto siempre desde `project.owner_user_id`, nunca desde `run.triggered_by_user_id` (null en escaneos por cron). Sin diff visual — el panel de la campana sigue leyendo los datos derivados de hoy; eso es la fase 1b. 751/751 tests, build/typecheck/lint limpios. Fase 1b (lectura + UI) pendiente de su propio Task Intake. Ver también el hallazgo P0 RECS-DEDUPE-1 más abajo, descubierto al probar esta fase en vivo |
