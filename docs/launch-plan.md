...
| 7b | GROWTH-2 | 🟡 En curso (2.1 ✅ mergeada #286; 2.2 en PR) | #286 | 2026-08-02 | Motor de posicionamiento orgánico SEO+GEO, continuación de GROWTH-1. Task Intake aprobado 2026-08-02 ("modo YOLO" con Human Gate manual en cada PR, verificación con `ux-pilot` antes de pasar de fase, 2.6 requiere su propia aprobación, 2.7 es trabajo del fundador). 7 sub-fases: **2.1 BASE-TECNICA ✅ mergeada (PR #286, squash `5cd3fda`)**: `verification.google` vía `GOOGLE_SITE_VERIFICATION`, `alternates.canonical` en las 10 páginas públicas que ya exportan `metadata` (excluye `/` y `/pricing`, client components pendientes de refactor), fechas reales en `sitemap.ts`, `feed.xml` (RSS 2.0), campos SEO opcionales en `lib/blog/posts.ts`, JSON-LD `Organization`/`BreadcrumbList`, runbook de Search Console/Bing. Journey de pilot nuevo (`tests/pilot/journeys/public-pages.spec.ts`) encontró un `PILOT FAIL` real (overflow 3px en `/blog` mobile) — arreglado (`overflow-x: hidden` en `html`+`body`, harness instrumentado para identificar el elemento culpable la próxima vez) y confirmado `PILOT PASS` antes del Human Gate. **2.2 ESTRATEGIA-AGENTES (en PR)**: `docs/content-strategy.md` (arquitectura de 5 capas, reglas de redacción SEO+GEO con densidad como techo no objetivo, cadencia), `docs/content-calendar.md` (ledger, primeras piezas priorizadas de las capas A/C/D/B), agente nuevo `seo-geo-research` (read-only, investiga y prioriza), `growth-content` ampliado con las reglas de redacción y el protocolo de refresco. Sin código de producto. Siguiente: 2.3 (`/docs`) → 2.4 (comparativas/glosario) → 2.5 (hubs) → 2.6 ⚠️ (Observatorio, aprobación propia) → 2.7 (off-site, fundador) |
