...
| 7b | GROWTH-2 | 🟡 En curso (2.1 en PR) | — | 2026-08-02 | Motor de posicionamiento orgánico SEO+GEO, continuación de GROWTH-1. Task Intake aprobado 2026-08-02 ("modo YOLO" con Human Gate manual en cada PR, 2.6 requiere su propia aprobación, 2.7 es trabajo del fundador). 7 sub-fases: **2.1 BASE-TECNICA** (en PR): `verification.google` vía `GOOGLE_SITE_VERIFICATION` (no-op sin la variable, mismo patrón que Sentry/PostHog), `alternates.canonical` en las 10 páginas públicas que ya exportan `metadata` (geo, privacidad, cookies, términos, índice de blog, 5 posts — **excluye `/` y `/pricing`**, que son client components y necesitan un refactor propio para soportar metadata por página, anotado como pendiente), fechas reales por ruta en `sitemap.ts` (antes `new Date()` en cada petición), `feed.xml` (RSS 2.0), campos `seoTitle`/`metaDescription`/`primaryKeyword` opcionales en `lib/blog/posts.ts`, JSON-LD `Organization` (montado una vez en el layout raíz, sin `sameAs` inventados) y `BreadcrumbList` en el blog. Runbook de alta en Search Console y Bing documentado en `docs/environment-contract.md` — requiere que el fundador pegue el token, no automatizable. **No toca** la casing `GenScore`/`Genscore` en cuerpo de texto (inconsistencia real y ya trazada a BRAND-5b/5c, fuera de alcance de esta fase — sí se corrigió la etiqueta `<title>` de los 5 posts y del índice del blog a "Genscore" para que coincida con el layout raíz, cero cambio de copy visible). Siguiente: 2.2 (estrategia + agentes) → 2.3 (`/docs`) → 2.4 (comparativas/glosario) → 2.5 (hubs) → 2.6 ⚠️ (Observatorio, aprobación propia) → 2.7 (off-site, fundador) |
