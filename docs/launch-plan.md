...
| 7b | GROWTH-2 | 🟡 En curso (2.1 ✅ #286; 2.2 ✅ #290; 2.3 ✅ #291; 2.4 en PR) | #286, #290, #291 | 2026-08-02 | Motor de posicionamiento orgánico SEO+GEO, continuación de GROWTH-1. Task Intake aprobado 2026-08-02 ("modo YOLO" con Human Gate manual en cada PR, verificación con `ux-pilot` antes de pasar de fase, 2.6 requiere su propia aprobación, 2.7 es trabajo del fundador). **2.1 BASE-TECNICA ✅ mergeada** (PR #286, squash `5cd3fda`): Search Console, canonicals, sitemap real, RSS, JSON-LD; journey de pilot nuevo encontró un `PILOT FAIL` real (overflow 3px en `/blog` mobile) — arreglado (`overflow-x: hidden` en `html`+`body`, harness instrumentado para identificar el elemento culpable la próxima vez). **2.2 ESTRATEGIA-AGENTES ✅ mergeada** (PR #290, squash `b5904f1`): `docs/content-strategy.md`, `docs/content-calendar.md`, agente `seo-geo-research`, `growth-content` ampliado con reglas de redacción SEO+GEO y protocolo de refresco. **2.3 DOCS-PUBLICOS ✅ mergeada** (PR #291): `/docs` — espejo del producto, mismo patrón que el Knowledge Base de Semrush, 5 páginas (índice + `empezar/primer-escaneo` + `informes/overview` + `metodologia/geo-score` trazado a ADR-0015 + `planes-y-limites`); QA encontró y corrigió etiquetas de franja de madurez inventadas antes del Human Gate. **2.4 GLOSARIO+COMPARATIVAS** (este PR, primer slice): `/glosario` (15 términos, `DefinedTermSet` JSON-LD, `lib/glosario/terms.ts` como SSOT) y `/comparativas/genscore-vs-otterly` (datos de Otterly de fuentes públicas de terceros con fecha de consulta visible, incluye honestamente 3 filas donde Otterly gana — multi-país, nº nominal de motores, usuarios ilimitados desde el plan de entrada). `genscore-vs-peec-ai` y la página pilar `mejores-herramientas-geo-en-espanol` quedan para una PR siguiente, no de una vez. Siguiente: resto de 2.4 → 2.5 (hubs) → 2.6 ⚠️ (Observatorio, aprobación propia) → 2.7 (off-site, fundador) |
