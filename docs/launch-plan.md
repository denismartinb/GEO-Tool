...
| 7b | GROWTH-2 | 🟡 En curso (2.1 ✅ mergeada #286; 2.2 ✅ mergeada #290; 2.3 en PR) | #286, #290 | 2026-08-02 | Motor de posicionamiento orgánico SEO+GEO, continuación de GROWTH-1. Task Intake aprobado 2026-08-02 ("modo YOLO" con Human Gate manual en cada PR, verificación con `ux-pilot` antes de pasar de fase, 2.6 requiere su propia aprobación, 2.7 es trabajo del fundador). **2.1 BASE-TECNICA ✅ mergeada** (PR #286, squash `5cd3fda`): Search Console, canonicals, sitemap real, RSS, JSON-LD; journey de pilot nuevo encontró un `PILOT FAIL` real (overflow 3px en `/blog` mobile) — arreglado (`overflow-x: hidden` en `html`+`body`, harness instrumentado para identificar el elemento culpable la próxima vez). **2.2 ESTRATEGIA-AGENTES ✅ mergeada** (PR #290, squash `b5904f1`): `docs/content-strategy.md`, `docs/content-calendar.md`, agente `seo-geo-research`, `growth-content` ampliado con reglas de redacción SEO+GEO y protocolo de refresco. **2.3 DOCS-PUBLICOS** (este PR, primer slice): `/docs` — espejo del producto, mismo patrón que el Knowledge Base de Semrush. 5 páginas: índice + `empezar/primer-escaneo` + `informes/overview` + `metodologia/geo-score` (trazado a ADR-0015, GEO Score v2: presencia .40/prominencia .25/cuota de voz .20/autoridad .15, confianza alta ≥20 resultados) + `planes-y-limites` (datos reales de `app/pricing/plans-data.ts`). Sidebar de navegación (`components/docs/docs-page-shell.tsx`, `lib/docs/nav.ts` como SSOT), canonical + `BreadcrumbList` en cada página, añadidas al sitemap. Journey de pilot nuevo (`tests/pilot/journeys/docs-pages.spec.ts`) + fixture extendido. El resto del mapa de `/docs` (informes de prompts/competidores/citations/recomendaciones, motores-y-cobertura, ciclo-de-escaneo) queda en `docs/content-calendar.md` (Capa A) para PRs pequeñas posteriores — no se ejecuta todo de una vez. Siguiente: 2.4 (comparativas/glosario) → 2.5 (hubs) → 2.6 ⚠️ (Observatorio, aprobación propia) → 2.7 (off-site, fundador) |
