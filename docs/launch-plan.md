...
| 10 | AUTH-EMAIL-VERIFY-1 | 🟡 Código en PR, falta activar el toggle | — | 2026-07-18 | Task Intake aprobado 2026-07-18. Signup con email/contraseña exige confirmación por enlace antes de dar acceso: `app/signup/actions.ts` detecta la ausencia de sesión de `signUp()` y manda a `/signup/confirm` en vez de `/dashboard`; `app/auth/callback/route.ts` envía el email de bienvenida al confirmar (reusa la detección de "cuenta recién creada" ya existente para OAuth); login con cuenta no confirmada da mensaje seguro en castellano (`error.code === "email_not_confirmed"`). Sin cambios de schema/RLS. **Pendiente y no shipeado en este PR**: activar "Confirm email" en el proyecto de Supabase real (dashboard, no repo) — hasta entonces el código es un no-op y el registro sigue funcionando exactamente igual que antes |
