...
| 10 | AUTH-EMAIL-VERIFY-1 | ✅ Hecho | #237 | 2026-07-19 | Task Intake aprobado 2026-07-18, PR #237 mergeado 2026-07-18. Signup con email/contraseña exige confirmación por enlace antes de dar acceso: `app/signup/actions.ts` detecta la ausencia de sesión de `signUp()` y manda a `/signup/confirm` en vez de `/dashboard`; `app/auth/callback/route.ts` envía el email de bienvenida al confirmar; login con cuenta no confirmada da mensaje seguro en castellano (`error.code === "email_not_confirmed"`), igual que el rate limit del mailer (`over_email_send_rate_limit`). Sin cambios de schema/RLS. Dos bugs reales encontrados y corregidos en vivo probando en producción antes del merge (sesión no comprobada por el código viejo, rate limit del mailer de pruebas de Supabase sin mapear). SMTP propio vía Resend configurado en Supabase 2026-07-19 (mismo remitente de pruebas `onboarding@resend.dev` que ya usa `lib/email/transactional.ts` — C3 del checklist de Fase 5, dominio propio `genscore.es` verificado en Resend, sigue pendiente). Verificado end-to-end en producción con el toggle activado: `/signup/confirm` → email de confirmación recibido en segundos → enlace lleva a `/dashboard` con sesión real. Toggle activado en producción de forma permanente |
| 11 | BRANDED-EMAILS-1 | ✅ Hecho | #242 | 2026-07-23 | Sistema de marca real para los 8 emails transaccionales (Resend) y las 2 plantillas de Supabase Auth (registro, recuperación de contraseña): cabecera de marca, tipografía, botones, paneles de dato; documento HTML completo por email (viewport, `color-scheme` fijado a claro, preheader) para compatibilidad multi-cliente; emojis con criterio en los emails más marketinianos; copy corregida en el aviso de caída de score; resumen semanal muy ampliado con datos reales de las 4 secciones del producto (sub-scores, actividad de la semana, competidores, recomendaciones) vía `lib/scan/weekly-digest.ts`, sin queries de escritura nuevas. Tras mergear BRAND-1/BRAND-2 (logo real G-gauge, ver `docs/brand/brand-guidelines.md`), pasada adicional (BRAND-3, parte de emails) para sustituir el placeholder por el logo real: `public/brand/genscore-logo-white-email.png`, rasterizado a 2x desde el SVG oficial vía Chromium headless (el SVG inline que usa la app no se renderiza en Outlook de escritorio). Bug real encontrado y corregido en el propio proceso de revisión: el fallback de confirmación de registro mostraba la URL de Supabase en crudo, ahora es un hipervínculo de texto. Sin cambios de schema/RLS/billing. Pendiente, fuera de alcance: conectar Open Graph/Twitter metadata (resto de BRAND-3, `docs/brand/brand-guidelines.md`) |
