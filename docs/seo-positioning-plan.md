...
| C3 | ✅ **Hecho (2026-08-12)** — `/comparativas/alternativas-a-otterly`. Organizada por el límite que empuja a buscar alternativa (tope de prompts, motores como add-on, ejecución, idioma), no por ranking. Declara en el primer bloque que la escribe un competidor, y cada alternativa —Genscore incluida— declara qué NO resuelve, con test que lo impone. Detalle: `docs/content-calendar.md`, log §63 | 2 | C/comparativas |
