...
import { restoreRecommendationCore } from "./restore-recommendation";
...
const RECOMMENDATION = { id: "rec-1", project_id: "project-1", status: "dismissed" };
const USER = { id: "user-1" } as unknown as AuthenticatedContext["user"];

describe("restoreRecommendationCore", () => {
  it("restores a dismissed recommendation to active via the service-role client", async () => {
    ...
    const result = await restoreRecommendationCore({
      projectId: "project-1",
    ...
    expect(updates).toEqual([{ id: "rec-1", project_id: "project-1", patch: { status: "active" } }]);
  });

  it("is idempotent: an already-active recommendation returns success without a redundant write", async () => {
  ...
  it("returns a sanitized error when the service-role update fails", async () => {
    ...
    expect(result).toEqual({
      success: false,
      error: "No se ha podido deshacer en este momento. Inténtalo de nuevo en unos minutos."
    });
  });
});
