...
export const GLOSSARY_TERMS: GlossaryTerm[] = [
  ...
  },
  {
    ...
    relatedLinks: [
      { href: "/docs/metodologia/geo-score", label: "Documentación: cómo pesa la cuota de voz en el GEO Score" },
      { href: "/glosario/geo-score", label: "Glosario: qué es el GEO Score" },
  ...
  },
  {
    ...
    longDefinition:
      "llms.txt es un fichero de texto en la raíz de un dominio (`/llms.txt`) que resume el sitio en lenguaje natural, pensado en teoría para que un modelo de IA lo consuma con facilidad al hacer grounding — el equivalente a robots.txt, pero dirigido a agentes de IA en vez de a rastreadores de búsqueda.\n\nConviene ser honesto sobre su utilidad real: la evidencia disponible hasta ahora no muestra que tener un llms.txt mejore la tasa de citación en respuestas de IA. Un estudio a gran escala sobre varios cientos de miles de dominios no encontró una correlación estadísticamente significativa entre tenerlo y ser citado más, y Google ha confirmado públicamente que no lo usa ni tiene previsto soportarlo. De los dominios más citados por motores de IA, la inmensa mayoría no tiene ningún llms.txt.\n\nGenscore lo trata como lo que es hoy: un estándar propuesto, todavía sin adopción real por parte de los motores generativos principales, no una palanca de citación. Lo que sí tiene evidencia de importar es lo de siempre — contenido claro, bien estructurado, con datos estructurados reales (`Article`, `FAQPage`, `DefinedTerm`) y autoridad de dominio genuina — no un fichero adicional que ningún rastreador de IA confirma leer todavía de forma consistente.",
    relatedLinks: [
