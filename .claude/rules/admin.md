...
- **Every write from `/admin` needs an email to `OPS_ALERT_EMAIL`.** There is
  ...
  surface has (`docs/brand/design-decisions-log.md` §97). It records who, which
  ...
  aware of the trade-off (§98). Do not reintroduce a reason requirement as a
...
- **An operator-scoped write may never have a precondition weaker than the
  ...
  (recurring scans on Free, §71; coverage audit below Pro, §97) — the pattern
...
- **Presentation helpers shared between the server page and a Client
  ...
  `cost-model.ts` (§98). If a future admin screen needs the same split,
