...
9. **Ask the founder whether to run the agentic user pilot**, and run it
   (`ux-pilot`, dispatched by hand) against the Vercel preview if they say yes.
...
**Desde VERCEL-COST-1 Fase 5 (2026-08-31) el piloto YA NO corre en cada deploy
de preview** (log §199). Se dispara sólo a mano (`workflow_dispatch` con el
número de PR), y el Director **tiene que preguntar al fundador antes de cada
Human Gate** si lo lanza. Sigue siendo obligatorio para dar un PR por
verificado; lo que cambia es quién decide cada pasada. Motivo, medido en la
factura real de agosto: cada pasada son ~6.750 peticiones contra el preview, y
las dos partidas mayores de la factura ($24,02 de Build CPU y $18,09 de
Observability Events, el 79% del total) son coste de este bucle agéntico, no de
clientes.
...
(`ux-pilot.yml` se dispara a mano, pero necesita igualmente que exista el
preview de ese commit para pilotarlo), así que saltar
...
**Se lanza a mano, y lo decide el fundador** (VERCEL-COST-1 Fase 5, 2026-08-31,
log §199). `ux-pilot.yml` es `workflow_dispatch` con el número de PR: ningún
deploy lo dispara. Antes de cada Human Gate el Director **pregunta** si hay algo
que mirar —muchos PRs no tocan UI— y sólo entonces lo lanza. Dos consecuencias
que hay que tener presentes y no maquillar:

1. **La ausencia de piloto ya no se ve.** Antes, un PR sin pasada no tenía
   check; ahora no tenerlo es lo normal. Si el Director no pregunta, nadie lo
   echa de menos. Por eso preguntar no es cortesía: es el único mecanismo que
   queda.
2. **"No hacía falta" es una respuesta legítima del fundador, no del Director.**
   El Director puede recomendar saltarlo y decir por qué; decidirlo solo, no.
...
The default read pilot (dispatched by hand since Fase 5, log §199 — hasta
...
- Perplexity runtime;
- crawler;
- background scheduler — **one exception granted**: TRIAL-REMINDER-3D-1
  (Task Intake approved 2026-09-19, see `docs/brand/design-decisions-log.md`
  §224), the daily `/api/cron/trial-reminders` cron that sends the
  "3 days before trial ends" reminder. Ships disabled by default
  (`CRON_TRIAL_REMINDER_ENABLED`), same kill-switch shape as every other
  cron here. Any other background scheduler still needs its own approval;
- billing — **partially approved**: BILLING-STRIPE-1 (Task Intake approved
  ...
  Resend (welcome, plan confirmed, payment failed, trial ended), built and
  tested against Stripe **test mode** only. Switching to live charges
  requires the go-live checklist in that same section (Vercel Pro, alta
  autónomo, VeriFactu decision) — still gated on founder sign-off, not
  automatic. Any billing work beyond that approved scope (new pricing
  mechanics, additional payment providers, invoicing changes) needs its own
  approval — approvals already granted: BILLING-INVOICE-FIELDS-1 (Task
  Intake approved 2026-08-25, see `docs/brand/design-decisions-log.md`
  §166), razón social/NIF synced to the Stripe customer's
  `invoice_settings.custom_fields`, still test mode only; TRIAL-REMINDER-3D-1
  (Task Intake approved 2026-09-19, see `docs/brand/design-decisions-log.md`
  §224), the "3 days before trial ends" reminder email — see the background
  scheduler exception above for its cron half;
- teams / RBAC;
- auto-merge;
...
|---|---|---|---|
| Competidores | `competitors.md` | **La frase larga del "puesto medio" (Amazon/Mozilla) se retira sin sustituto, mal maquetada según el fundador — el rótulo "Puesto medio" se queda solo (2026-09-09, log §212, corrige parcialmente §177)** · SCREEN-POLISH-1 Fase A: "tema" (ya usado aquí en "Terreno por tema") se adopta también en Prompts para "categoría", unificando el vocabulario de esa pantalla (2026-09-06, log §205) · ENTITY-HYGIENE-1: un asistente de IA ("ChatGPT") nunca es un competidor y un término genérico del sector ("GEO Score") nunca es un alias de marca — cinco puntos de entrada cerrados (2026-08-30, log §200) · TRUST-METRICS-1: '45 prompts' pasa a '45 respuestas · 15 prompts en 3 motores', y un motor con 0 menciones vuelve a la tabla en vez de desaparecer (2026-08-27, log §183) · MEAN-RANK-READS-TRUE-1: «Puesto» pasa a «Puesto medio» y el gráfico se ordena por la tabla que tiene debajo (2026-08-27, log §177; la frase explicativa que pegaba a la cifra se retiró en §212) · SAMPLE-FLOOR-1: una media sobre pocas respuestas deja de adelantar a una sobre muchas (2026-08-27, log §175) · ANIMATION-PARITY-1: la misión del primer escaneo se ve y termina igual en las 6 secciones (2026-08-26, log §168) · SCAN-FULLSCREEN-HEADER-1: la cabecera fija se oculta mientras dura la misión del primer escaneo (2026-08-25, log §160) · ONBOARDING-COMPETITORS-CAP-1 (2026-08-20, log §123) · PANORAMA-EMPTY-1 (2026-08-07) | log §10, §11, §15, §36, §123, §160, §168, §175, §177, §183, §200, §205, §212 · ADR 0011/0018/0020/0022 · `docs/external-audit-2026-08.md` |
| Recomendaciones | `recommendations.md` | **PDF-EXPORT-PLAN-1: Safari iOS ignora `@page { margin: 0 }` y reserva sitio para su cabecera/pie, así que una altura fija en px NUNCA cabe — la portada pasa a `height: 100%` y cuelga directa de `body`; la fila de metadatos ya no se cae a una segunda página (2026-09-12, log §219)** · PDF-EXPORT-PLAN-1: la portada ocupaba página y media — faltaba `box-sizing: border-box` (la caja medía 1235px, no 1123px); se rehace la paginación entera con `@page` con nombre para el contenido, y el degradado del artboard pasa a PNG incrustado (2026-09-11, log §218)** · PDF-EXPORT-PLAN-1: la portada se deformaba en un dispositivo real (layout de consola con `overflow`/`transform` como ancestro) — el informe se monta por `createPortal` en `document.body`, y el resplandor decorativo pasa de degradado CSS a SVG, más fiable al exportar a PDF (2026-09-11, log §217)** · PDF-EXPORT-PLAN-1 corrección: logo oficial, medidas px pixel-perfect contra el artboard, y `.xrp-content` deja de tener altura fija — una lista larga de recomendaciones se recortaba en silencio en vez de fluir a más páginas (2026-09-11, log §216)** · PDF-EXPORT-PLAN-1: "Exportar plan" deja de descargar un `.md` y pasa a abrir el diálogo de impresión del navegador sobre un informe con marca ("Informe de consultoría", diseño aprobado en `docs/design-reference/pdf-export-plan-1/`) — el `.md` conserva su generador sólo como contenido del modal de respaldo (2026-09-11, log §215)** · ACTIONS-OBSERVABLE-1 slice 4b: "Exportar plan" pasa por el contrato de acción, con modal de respaldo si la descarga falla — la última acción de la Fase 4 que queda como CTA (2026-09-08, log §210; su otra mitad, "Activar seguimiento diario", se retiró horas después en vez de arreglarse, ver zona "Dominios y depuración" §211)** · ACTIONS-OBSERVABLE-1 slice 4a: "generar" y "marcar como hecho" pasan por un contrato de acción compartido; el "Deshacer" de la segunda vive de forma durable en "Resueltas", no en la tarjeta activa (2026-09-07, log §203) · ENTITY-HYGIENE-1: "Seguir competidor emergente" deja de poder recomendar un asistente de IA (2026-08-30, log §200) · La pestaña "Resueltas" tenía el icono de check desalineado del texto — `.rec-main` esperaba un solo hijo tras RECS-REDESIGN-1 (2026-08-29, log §196) · RECURRING-VALUE-1: la tarjeta activa dice "La verás reflejada en tu próximo escaneo" junto a "Marcar como hecho" — primer recorte de Fase 3 de la auditoría externa (2026-08-29, log §195) · RECS-EVIDENCE-2: cada tarjeta dice qué motor la respalda, y dos motores de acuerdo sobre el mismo prompt ya no se pisan entre sí — Fase 7 de la auditoría externa (2026-08-28, log §191) · RECS-LOOP-1 Fase B: la brecha marcada como hecha que reaparece deja de verse como nueva — insignia en la tarjeta activa + veredicto en el historial, mismo ancla fijo que Fase A (2026-08-28, ADR 0041 adenda, log §190) · RECS-LOOP-1 Fase A: la pestaña "Resueltas" verifica si la predicción se cumplió, nunca un delta de score (2026-08-27, ADR 0041, log §181) · AUDIT-RECS-JOIN-1 Fase B (2026-08-27, log §172) · ANIMATION-PARITY-1: la misión del primer escaneo se ve y termina igual en las 6 secciones (2026-08-26, log §168) · AUDIT-RECS-JOIN-1 Fase A (2026-08-22, log §167) · SCAN-FULLSCREEN-HEADER-1: la cabecera fija se oculta mientras dura la misión del primer escaneo (2026-08-25, log §160) · RECS-ACCION-1c (2026-08-21, log §140) · REWRITE-DOMAIN-ANCHOR-1 (2026-08-20/21, log §137, §133 y §134) · RECS-USEFULNESS-1 Fase C: honestidad de lo generado (2026-08-21, log §128) · RECS-ACCION-1a (2026-08-20, log §127) · RECS-USEFULNESS-1 Fase A: integridad del artefacto pegable (2026-08-20, log §126) · RECS-REDESIGN-1 fase 1 (2026-08-17, log §115) · RECS-POTENTIAL-1 (2026-07-23) | log §115, §126, §127, §128, §133, §134, §137, §140, §160, §167, §168, §172, §181, §190, §191, §195, §196, §200, §203, §210, §215, §216, §217, §218, §219 · ADR 0017/0019/0041 · `docs/specs/recommendations/quality-audit-2026-08.md` · `docs/external-audit-2026-08.md` |
| Auditoría web | `web-audit.md` | **Fase 5 de la auditoría externa se cierra sin devolver "Auditar ahora" — riesgo aceptado por el fundador, premisa corregida (`/debug` sí puede apagarla hoy), mitigado por reintentos + alerta al operador ya existentes (2026-09-07, log §209)** · **WEB-AUDIT-WIDTH-1: la columna sube los cuatro peldaños de la escalera de anchos, no sólo el primero (2026-08-27, log §178)** · ANIMATION-PARITY-1: la misión del primer escaneo se ve y termina igual en las 6 secciones (2026-08-26, log §168) · Retirado el pie "Dominios/Recomendaciones", redundante con la navegación lateral (2026-08-25, log §164) · SCAN-FULLSCREEN-HEADER-1: la cabecera fija se oculta mientras dura la misión del primer escaneo (2026-08-25, log §160) · AUDIT-SNIPPET-1 (2026-08-21, log §131) · AUDIT-GROUNDED-PARITY-1 (2026-08-21, log §130) · El beat de ascenso llega a esta pantalla (2026-08-20, log §132) · PRELAUNCH-HARDENING-1 Fase R7-b (2026-08-16, log §106) · Fase R8 (2026-08-15, log §102) · tests de render (2026-08-14, log §87) · Fase R7 (2026-08-14, log §83) · SCAN-STATES-3 (2026-08-11, log §57) · WEB-AUDIT-AUTO-SPLIT-1 (2026-08-09) · WEB-AUDIT-DRIVE-1 (2026-08-07, ADR 0038) · WEB-AUDIT-TECH-ALL-PLANS-1 (2026-08-05, ADR 0035) | log §17, §18, §22, §25, §27, §30, §52, §57, §83, §87, §102, §106, §130, §131, §132, §160, §164, §168, §178, §209 · ADR 0027/0035/0038 · `docs/specs/web-audit/ROADMAP.md` |
...
| Escaneo (pipeline) | `scan.md` · `mission-rocket.md` | **RECURRING-CADENCE-1 Fase B: el barrido avisa por correo al operador de lo que la alerta por-run no podía ver —escaneo del cron que revienta, proyecto expulsado por racha de fallos, pasada sin progreso, cadena rechazada— con un correo por pasada, sin migración (2026-08-30, log §194)** · RECURRING-CADENCE-1 Fase A: la elegibilidad del escaneo recurrente se ancla al horario del cron —un escaneo manual dejaba al proyecto sin el automático del día siguiente—, el barrido pregunta si el lote cabe ANTES de empezarlo, y su continuación comprueba `response.ok` (2026-08-29, log §192)** · RECS-FINALIZE-DURABILITY-1: el borrado de las recomendaciones del propio run pasa a filtrar por `status='active'` y comprobar su error, como sus dos vecinos — no había fuga activa, era un peligro latente (2026-08-28, log §188)** · CITATION-REDIRECT-SSRF-1: `resolveGroundingRedirect` verifica cada salto de redirección antes de seguirlo, guardián importado de `fetch-page.ts` (2026-08-27, log §185) · PROJECT-DEFAULTS-BY-ACCOUNT-1: `recurring_scans_enabled` se activa solo tras el primer escaneo completado de una cuenta no excluida (2026-08-25, log §173) · PRELAUNCH-HARDENING-1 Fase Q3: tests de ruta del cron (2026-08-15, log §90) · Fase R6 (2026-08-14, log §81/§82) · ENGINE-DEBUG-TOGGLE-1 (2026-08-10) · SAMPLING-DEBUG-TOGGLE-1 (2026-08-09) · SCAN-DRIVE-1 (2026-08-07, ADR 0037) · EXTRACTION-RELIABILITY-1 Fase C (2026-08-05) | log §53, §54, §81, §82, §90, §173, §185, §188, §192, §194 · `docs/scan-lifecycle.md` · ADR 0003/0006/0014/0016/0029/0037 |
| Dominios y depuración  | — *(sin regla propia todavía)*  | **El botón "Activar seguimiento diario" del banner de madurez de datos se retira en vez de arreglarse — el fundador no quiere ese CTA, ni hoy ni tras el default de lanzamiento (2026-09-09, log §211)** · SCREEN-POLISH-1 Fase B: el asistente pedía 15 prompts recomendados pero sólo generaba 10 — el tope sube a 15 y el copy pasa a ser consciente del plan del usuario (2026-09-07, log §208)** · TRUST-METRICS-1: la puntuación pasa a leer la ventana (antes visibilidad cruda), su delta pasa a ventana-sobre-ventana, y `/runs/[runId]` sale de la consola — sólo accesible desde `/debug` (2026-08-27, log §183)** · **MATURITY-BANNER-HIDE-ALL-1: el switch de `/debug` silencia todos los avisos informativos —la puerta enumera excepciones, no cubiertos— y el aviso del plan Free nunca se calla (2026-08-27, log §179)** · PROJECT-DEFAULTS-BY-ACCOUNT-1: sampling, auditoría por IA y los avisos de historial en construcción nacen en ON/ocultos para cuentas reales, salvo un allow-list de cuentas internas de prueba (2026-08-25/27, log §173) · DOMAINS-OVERAGE-GATE-1: gate bloqueante en toda la consola cuando la cuenta tiene más dominios activos de los que permite el plan — retirar (borrado duro) o subir de plan (2026-08-25, log §171) · Retirado el enlace "Volver a competidores" del paso de prompts, redundante con el botón "Atrás" del propio formulario (2026-08-25, log §164) · DEBUG-HIDE-NO-TRACKING-1: switch local en `/debug` para silenciar el aviso de seguimiento diario (2026-08-25, log §161) · ONBOARDING-DOMAIN-REDESIGN-1: rediseño del asistente de alta de dominio a la dirección "Consola" (2026-08-20, log §162) · DOMAINS-LIVE-SELECT-1 (2026-08-20, log §122) · DOMAINS-ARCHIVE-RETIRE-1 (2026-08-15, log §104) · PRELAUNCH-HARDENING-1 Fase Q1: `createProjectCore` + tests (2026-08-15, log §89) · DOMAINS-CLIENT-DELETE-1 (2026-08-09) · DOMAINS-ACTIVE-COOKIE-1 (2026-08-07) · FAVICON-QUALITY-1 Fases 1 y 3a (2026-08-06) · DEBUG-ACTIVE-PROJECT-1 (2026-08-06) · DOMAINS-REDESIGN-1 Fase A (2026-08-05) | log §32, §33 (DEBUG), §36, §39, §41, §89, §104, §122, §161, §162, §164, §171, §173, §179, §183, §208, §211 · `docs/design-reference/domains-redesign-1/` · `docs/design-reference/onboarding-domain-redesign-1/` · `docs/external-audit-2026-08.md` |
| Visión general   | — *(sin regla propia todavía)*   | **GEO-BAND-ALWAYS-1: la franja cualitativa ya no se oculta con muestra insuficiente — sólo la franja, el delta y el sparkline siguen gateados igual que antes (2026-09-12, log §220, founder-approved)** · **RECURRING-VALUE-1: "la tendencia estará disponible con ≥2 escaneos" desaparece sin sustituto — primer recorte de Fase 3 de la auditoría externa (2026-08-29, log §195)** · **TRUST-METRICS-1: la frase del resumen deja de enseñar una segunda 'puntuación GEO' distinta de la del medidor (2026-08-27, log §183)** · **MEAN-RANK-READS-TRUE-1: «Puesto» pasa a «Puesto medio», con la frase que lo explica pegada a la cifra, y el gráfico se ordena por la tabla que tiene debajo (2026-08-27, log §177)** · SAMPLE-FLOOR-1: una media sobre pocas respuestas deja de adelantar a una sobre muchas (2026-08-27, log §175) · ANIMATION-PARITY-1: la misión del primer escaneo se ve y termina igual en las 6 secciones (2026-08-26, log §168) · SCAN-FULLSCREEN-HEADER-1: la cabecera fija se oculta mientras dura la misión del primer escaneo (2026-08-25, log §160) · HEADER-FULL-WIDTH-1: la cabecera de página ya no se recorta en ventanas anchas (2026-08-21, log §150) · OV-DESKTOP-2: cabecera alineada, desglose 2/3 + motores, filas con icono/medidor (2026-08-20, log §119) · Retirada la banda «Revisando tu web» (2026-08-16, log §111) · Corrección del banner `scan_started` stale (2026-08-16, log §112) · PRELAUNCH-HARDENING-1 Fase R7 2/2 (2026-08-14, log §84) · SCAN-STATES-2 (2026-08-10) · ONBOARDING-ROCKET-1 Fase 1 (2026-08-08) | log §4, §6, §8b, §55, §56, §84, §112, §111, §119, §150, §160, §168, §175, §177, §183, §195, §220 · `docs/external-audit-2026-08.md` |
| Prompts | — *(sin regla propia todavía)* | **SCREEN-POLISH-1 Fase A: el sentimiento sólo se cuenta sobre filas con `brand_mentioned === true` (`lib/metrics/brand-sentiment.ts`, dueño único junto con Overview y por-motor), las filas de prompt/tema se pueden abrir con teclado, y "Topics"/"categoría" pasan a "tema" (2026-09-06, log §205)** · ANIMATION-PARITY-1: la misión del primer escaneo se ve y termina igual en las 6 secciones (2026-08-26, log §168) · SCAN-FULLSCREEN-HEADER-1: la cabecera fija se oculta mientras dura la misión del primer escaneo (2026-08-25, log §160) · PROMPT-DRAWER-TRUTH-1 (2026-08-23, log §147) · SAMPLING-SURFACE-1 (2026-08-05) | log §5, §24, §147, §160, §168, §205 · ADR 0021/0030 |
| Páginas citadas | `citations.md` | **CITED-DIFF-1 Fase 0: script de validación barata (`pnpm cited-diff:validate`, sólo ejecutable por el fundador) antes de construir la fase permanente — aún sin aprobar (2026-08-28, log §186)** · **CITATIONS-HONESTY-1: "cita a un rival" pasa a "citada en una respuesta donde también apareció X", el filtro de outreach deja de exigir competidor co-citado y las fuentes alcanzables se agrupan por dominio (2026-08-27, Fase 8 de la auditoría externa, log §189)** · ANIMATION-PARITY-1: la misión del primer escaneo se ve y termina igual en las 6 secciones (2026-08-26, log §168) · SCAN-FULLSCREEN-HEADER-1: la cabecera fija se oculta mientras dura la misión del primer escaneo (2026-08-25, log §160) · HEADER-FULL-WIDTH-1: la cabecera adopta el patrón compartido de las otras 6 pantallas (2026-08-25, log §151) · Fix: tarjeta de Impacto recortada sin donut (2026-08-17, log §114) · CITATIONS-REDESIGN-1 (2026-08-01) | log §8, §39, §114, §151, §160, §168, §186, §189 · ADR 0010/0012/0013/0023 · `docs/external-audit-2026-08.md` Fase 8 |
...
| Correos transaccionales | — *(sin regla propia todavía)* | **TRIAL-REMINDER-3D-1: aviso nuevo 3 días antes de que termine el trial de Pro, empujado por un cron diario (`/api/cron/trial-reminders`, nace apagado) porque el resto de billing es perezoso y este caso no puede serlo — reutiliza `planPromoCard` con tono "Seguir con Pro", nunca "Activar"; idempotencia en `profiles.trial_reminder_sent_at` (migración 0036), escrita DESPUÉS de enviar a propósito (2026-09-19, log §224)** · TRIAL-EMAIL-PROMO-1: el email de fin de prueba enseña las dos promos vivas (Pro y Starter) con urgencia por fecha — ningún precio ni fecha se escribe a mano, todo se lee de `PLANS`/`isPromoActive`/`PROMO_ENDS_AT` (`app/pricing/plans-data.ts`), mismo patrón que TRUST-PROMISES-1; sin promo activa cae al aviso neutro original (2026-09-19, log §223)** · WEEKLY-DIGEST-VISUAL-1: barra de color proporcional bajo la puntuación del resumen semanal y delta semana-sobre-semana en cada sub-score — ningún dato nuevo, sólo hacer visible lo que `runWeeklyDigest` ya calculaba (2026-09-07, log §204)** · Un comentario de código con forma JSX (`{/* ... */}`) escrito dentro del template literal de HTML del resumen semanal salía tal cual, visible, en el correo del cliente — movido a JSDoc real fuera del literal, con test de regresión (2026-09-07, log §202) · RECURRING-CADENCE-1 Fase B: dos alertas de operador nuevas —resumen del barrido recurrente y cadena de continuación rechazada—, siempre a `OPS_ALERT_EMAIL`, nunca al cliente (2026-08-30, log §194)** · TRUST-METRICS-1: la alerta de caída y el resumen semanal dejan de llamar "GEO Score" a un compuesto por-run que no es la ventana — sólo copy, el cálculo de ninguno de los dos se toca (2026-08-27, log §183)** · WELCOME-EMAIL-FRESHNESS-FIX-1 (2026-08-20, ADR 0040) · PRELAUNCH-HARDENING-1 Fase Q2: primeros tests (2026-08-15, log §93) | ADR 0040 · log §93, §183, §194, §202, §204, §223, §224 · `lib/email/transactional.ts` · `docs/external-audit-2026-08.md` |
...
| Ajustes de cuenta | — *(sin regla propia todavía)* | **PRECIO-BUTTONS-CONSOLE-1 Fase 1: `/dashboard/settings` acepta `?openPlan=<id>`, validado contra `PLANS`, y abre "Cambiar de plan" ya preseleccionado — deep-link desde `/pricing` estando logado (2026-09-11, log §221)** · Los tres botones de "Plan y facturación" se apilan a ancho completo en móvil en vez de envolver desequilibrados (2026-09-06, log §201) · PROMO-CONSOLE-PARITY-1: la consola cotiza el precio de lanzamiento a quien todavía no ha contratado (2026-08-27, log §170) · BILLING-INVOICE-FIELDS-1: razón social/NIF sincronizan con Stripe al guardar (2026-08-25, log §166) · "Datos de empresa" oculto en Cuenta: no tenía consumidor ni plan (2026-08-25, log §165) · PRELAUNCH-HARDENING-1 Fase R8-a (2026-08-15, log §102) · CONSOLE-REDESIGN-1 Fase A (2026-08-06) | log §38, §102, §165, §166, §170, §201, §221 · `docs/design-reference/console-redesign-1/` |
| Precios y facturación | `styles.md` | **BILLING-COMPED-1 Fase 1: cuentas comped (allow-list por email, `COMPED_ACCOUNT_EMAILS`) se leen como plan Agency en todo punto que resuelve el plan, sin tocar Stripe ni escribir en `profiles.current_plan` — Fase 2 (pasar `STRIPE_SECRET_KEY` a live) queda tal cual, bloqueada por el checklist de `docs/launch-plan.md` Fase 4 (2026-09-18, log §222)** · PRECIO-BUTTONS-CONSOLE-1 Fase 1: los CTA de `/pricing` estando logado llevan directo al modal "Cambiar de plan" de la consola en vez de a `/signup` — `/pricing` sigue estática, la sesión se lee en cliente como ya hacía `PublicHeader` (2026-09-11, log §221; Fase 2, copy del botón consciente de trial, sin aprobar)** · La promo de lanzamiento se extiende hasta el 30 de septiembre — decisión del fundador; el cupón real de Stripe necesita el mismo cambio de `redeem_by` a mano, fuera de este repo (2026-09-07, log §206) · El test de la promo de Checkout dependía del reloj real y rompió al cerrarse la ventana el 2026-09-01 — `isPromoActive()` pasa a mockearse en vez de leer la fecha real (2026-09-01, log §197) · TRUST-PROMISES-1: los precios citados fuera de la consola (hero, metadescripción de /precios, comparativas) leen `plans-data.ts` en vez de repetir el número a mano (2026-08-27, log §182) · PRICING-PAY-BADGES-CENTER-1: las insignias de pago se centran también cuando envuelven en móvil, no sólo en una línea (2026-08-27, log §180) · PROMO-CONSOLE-PARITY-1: la consola cotiza el precio de lanzamiento a quien todavía no ha contratado (2026-08-27, log §170) · DOMAINS-OVERAGE-GATE-1: al exceder el cupo de dominios del plan, la salida a "subir de plan" reutiliza el checkout/portal de Stripe ya existente desde dentro del gate bloqueante (2026-08-25, log §171) · BILLING-INVOICE-FIELDS-1: razón social/NIF de "Datos de facturación" llegan a la factura vía `invoice_settings.custom_fields` de Stripe (2026-08-25, log §166) · PRICING-PROMO-1 Fase C: cupón real de Stripe, con fecha de caducidad real leída de la propia suscripción — no un número de pantalla (2026-08-25, log §152) · Fase A+B: copy de "paga por valor", medios de pago (2026-08-24, log §148, §149) | log §148, §149, §152, §166, §170, §171, §180, §182, §197, §206, §221, §222 · `docs/launch-plan.md` Fase 4 (BILLING-STRIPE-1) · `docs/external-audit-2026-08.md` |
...
| `competitors.md` | `app/dashboard/projects/*/competitors/**`, `lib/competitors/**`, `lib/brand-aliases/**`, `lib/projects/brand-aliases.ts`, `lib/entity-hygiene/**` |
