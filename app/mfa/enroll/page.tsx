...
export default async function MfaEnrollPage({
  ...
  // Widened to a plain string: TS 5.5's inferred-predicate narrowing on the
  // first `.find()` below otherwise collapses `factor.status`'s type to the
  // single literal it just matched, making the second `.find()`'s
  // "unverified" comparison a type error even though the runtime array can
  // hold either status.
  ...
  const pending = totpFactors.find((factor) => factor.status === "unverified");
      ...
      <div className="auth-card fade-in" style={{ maxWidth: 440 }}>
        ...
        {enrollFailed ? (
          <p className="feedback error" style={{ marginTop: 20 }}>
            No se pudo generar el código. Recarga la página para reintentarlo.
          </p>
        ) : (
          <>
            {params.error ? (
