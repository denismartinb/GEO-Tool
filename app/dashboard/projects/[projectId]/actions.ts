...
async function executePendingScan({
  ...
  try {
    ...
    const { data: promptResults } = await service
      .from("scan_prompt_results")
      .select("id, prompt_text_snapshot, brand_mentioned, citation_found, mentioned_competitors_count, citations_count, sentiment, extracted_json, extraction_error, status")
      .eq("project_id", projectId)
      .eq("run_id", runId);
